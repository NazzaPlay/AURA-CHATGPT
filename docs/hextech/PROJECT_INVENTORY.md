...
**Último commit**: `b3704c8` - "H1.5 verificar consistencia de jaula Hextech"
**Estado Git**: Clean (working tree clean, branch main actualizada con origin/main)
...
### Importaciones críticas detectadas (según documentación)
- Agentes de routing dependen de RN
- Core agent coordina múltiples agentes especializados
- Model registry gestiona proveedores de modelos
- Memory agent interactúa con memory_store.py

### Relaciones arquitectónicas
1. **main.py** → **aura.py** → **agents/core_agent.py**
2. **core_agent.py** → **agents/routing_*.py** → **backend/app/routing_neuron/**
3. **model_runner.py** → **providers/** → **agents/model_registry.py**
4. **memory_agent.py** → **memory_store.py** → **memory.json**
