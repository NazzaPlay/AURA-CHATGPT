...
- Los 3 Safetensors (22.62 GB) son candidatos a conversión futura en H3.9+
...
| Fase | Acción | Estado |
|------|--------|--------|
| H3.1 | Inventariar modelos en `A:\AURA\models` (solo lectura) | ✅ Completado |
| H3.5 | Model Profiler / Benchmark Design (diseño documental) | ✅ Completado |
| H3.9+ | Evaluar conversión de modelos no-GGUF a GGUF | Futuro |
...
- La conversión solo se hará con plan aprobado (H3.9+)
...
- **H3.5**: Model Profiler / Benchmark Design (diseño documental)
...
- **H3.9**: Conversión GGUF / Model Bank
...
- **Fase estimada**: H3.9+ (diseño en H3.5)
...
```
H3.0 → H3.1 → H3.2 → H3.3 → H3.4 → H3.5 → H3.6 → H3.7 → H3.8 → H3.9
 │       │       │       │       │       │       │       │       │       │
 │       │       │       │       │       │       │       │       │       └── Conversión GGUF
 ...
 │       │       │       │       │       │
 │       │       │       │       │       └── Model Profiler / Benchmark Design ✅
 │       │       │       │       │
...
#### H3.5 — Model Profiler / Benchmark Design ✅
- **Objetivo**: Diseñar conceptualmente un sistema de profiling/benchmark de modelos para medir y comparar modelos locales GGUF antes de asignarlos a roles en el routing multimodelo
- **Entregables**: Documento de diseño conceptual (`MODEL_PROFILER_DESIGN.md`)
- **Restricciones**: Solo diseño documental, sin implementación, sin benchmarks reales, sin tocar modelos
- **Duración estimada**: 1 sesión
- **Documento**: `docs/hextech/MODEL_PROFILER_DESIGN.md`
- **Estado**: ✅ Completado
- **Nota**: El "AURA Self-Work Loop controlado" originalmente planificado como H3.5 se ha movido a H3.6 para mantener coherencia con el roadmap documental. H3.5 ahora se enfoca exclusivamente en profiling/benchmark de modelos, que es un prerrequisito lógico antes de implementar el Self-Work Loop.

#### H3.6 — AURA Self-Work Loop controlado (movido desde H3.5)
...
- **Nota**: Esta fase fue originalmente H3.5. Se movió a H3.6 para que H3.5 sea el diseño de profiling/benchmark de modelos, que es prerrequisito lógico.

#### H3.7 — Pruebas y criterios de seguridad (movido desde H3.6)
...
#### H3.8 — RN Family: primeras neuronas (movido desde H3.7)
...
#### H3.9 — Conversión GGUF / Model Bank (movido desde H3.8)
...
- [ ] **Plan H3 completo aprobado** — Todas las fases H3.0 a H3.9 documentadas y aprobadas
...
| `MODEL_PROFILER_DESIGN.md` | Diseño conceptual del sistema de profiling/benchmark de modelos (H3.5) |
...
2. **No se implementa nada** que no esté en una fase H3.x  
aprobada  
3. **No se conecta DeepSeek API** sin plan H3.2 aprobado  
4. **No se convierten modelos** sin plan H3.9+ aprobado  
5. **No se modifican archivos protegidos** (backend/, agents/, providers/, etc.) sin permiso expl�cito  
  
### 16.2 Reglas de documentaci�n  
  
1. Este documento es **solo planificaci�n conceptual**  
2. No implica autorizaci�n para implementar  
3. Cada fase H3.x requiere su propio plan y aprobaci�n  
4. Las neuronas RN Family listadas son **candidatas conceptuales**, no compromiso de implementaci�n 
  
  
### 16.3 Reglas de prioridad  
  
1. **Seguridad  No sacrificar seguridad por velocidad  
2. **Documentaci�n  Primero documentar, luego implementar  
3. **Validaci�n  Validar antes de ejecutar cualquier cambio  
...
"|-------|---------|--------|"  
"| 27/4/2026 | v1.0 | Creaci�n inicial como parte de H3.0 |"  
"| 27/4/2026 | v1.1 | Actualizaci�n: H3.5 ahora es Model Profiler / Benchmark Design. Fases H3.6-H3.9 renumeradas. Agregada referencia a MODEL_PROFILER_DESIGN.md |"  
  
---  
  
"**Ubicaci�n**: \`docs/hextech/RN_SELF_WORK_PLAN.md\`"  
"**Responsable**: Cline bajo supervisi�n Hextech"  
"**Estado**: Documentaci�n conceptual (H3.0)"  
"**Pr�xima fase**: H3.6 - AURA Self-Work Loop controlado" 
